...
/// \brief 센서 값을 읽을 때 Emitter(적외선 발광부)의 동작 방식입니다.
///
/// Emitter 제어는 setEmitterPin()으로 유효한 핀을 지정해야만 작동합니다.
/// 홀수/짝수 모드는 두 개의 Emitter 제어 핀을 가진 2세대 QTR 또는 QTRX 센서를 사용하고,
/// setEmitterPins()로 두 핀을 모두 지정한 경우에만 작동합니다.
enum class QTRReadMode : uint8_t {
  /// 적외선(IR) Emitter를 켜지 않고 값을 읽습니다.
  /// 센서 주변의 주변광(Ambient Light) 수준을 나타냅니다.
  Off,

  /// Emitter를 켠 상태로 값을 읽습니다.
  /// 반사율(Reflectance)을 측정합니다.
  On,

  /// 각 센서마다 Emitter를 켠 상태와 끈 상태 두 번 값을 읽습니다.
  /// 반환되는 값은 **on + max &minus; off** 입니다.
  /// 여기서 **on**과 **off**는 각각 Emitter를 켰을 때와 껐을 때의 값이며,
  /// **max**는 센서가 읽을 수 있는 최댓값입니다.
  /// 이 모드는 불균일한 주변 조명으로 인한 간섭을 줄일 수 있습니다.
  OnAndOff,

  /// 홀수 번째 Emitter를 켜고 홀수 번째 센서를 읽은 다음,
  /// 짝수 번째 Emitter를 켜고 짝수 번째 센서를 읽습니다.
  /// 이 모드는 특히 QTRX 센서 보드에서 인접한 센서 간의 간섭을 줄일 수 있습니다.
  /// 두 개의 Emitter 제어 핀이 있는 2세대 QTR 및 QTRX 센서 어레이에서만 사용할 수 있습니다.
  OddEven,

  /// 홀수 및 짝수 센서를 각각 해당 Emitter를 켠 상태로 읽은 다음,
  /// Emitter를 끈 상태로 모든 센서를 읽고 **on + max &minus; off**를 반환합니다.
  /// (즉, 이 모드는 OddEven 모드와 OnAndOff 모드를 결합한 것입니다.)
  OddEvenAndOff,

  /// 이 모드로 read()를 호출하면 Emitter를 자동으로 제어하지 않습니다.
  /// Emitter는 현재 상태 그대로 유지되므로, 테스트나 고급 사용을 위해 수동으로 제어할 수 있습니다.
  /// 이 모드에서는 calibrate() 및 readCalibrated()가 지원되지 않습니다.
  Manual
};
...
/// 정의되지 않은 Emitter 제어 핀을 나타냅니다.
const uint8_t QTRNoEmitterPin = 255;

/// RC 센서의 기본 타임아웃 시간(마이크로초)입니다.
const uint16_t QTRRCDefaultTimeout = 2500;

/// 이 클래스의 인스턴스에서 지원하는 최대 센서 수입니다.
const uint8_t QTRMaxSensors = 31;

/// \brief QTR 센서 어레이를 나타내는 클래스입니다.
///
/// 이 클래스의 인스턴스는 동일한 타입의 센서 하나 이상으로 구성된 QTR 센서 어레이를 나타냅니다.
/// 단일 QTR 센서 보드일 수도 있고, 그룹으로 제어되는 여러 보드일 수도 있습니다.
///
/// 이 라이브러리 사용법에 대한 개요와 예제 코드는 \ref md_usage 를 참조하세요.
class QTRSensors
{
  public:
