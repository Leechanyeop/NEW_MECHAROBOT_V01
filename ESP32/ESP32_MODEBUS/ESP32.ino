...
/**
 * @file ESP32.ino
 * @brief Robot Control via Modbus TCP (OPC UA Gateway Integrated)
...
enum RegisterIndex {
  ...
  // REG_TARGET_A = 6,   // Now handled as Coil 00007 (Addr 6)
  REG_CONTROL = 99,      // 40100
  REG_STATE = 199        // 40200
};
...
void handleCommand(const char* cmdStr) {
  ...
  if (cmd == "1" || cmd == "MOVE" || cmd == "PID") {
    targetMode = 1; travelDist = 0.0f;
    sendSPI("<MOVE>");
    stringToMB("RUN", &holdingRegisters[REG_STATE]);
  }
  else if (cmd == "x" || cmd == "STOP" || cmd == "ESTOP") {
    targetMode = 0;
    if (cmd == "ESTOP") {
        sendSPI("<ESTOP>");
        stringToMB("EMERGENCY_STOP", &holdingRegisters[REG_STATE]);
    } else {
        sendSPI("<STOP>");
    ...
    targetMode = 0;
    sendSPI("<LEFT>");
    stringToMB("TURN_LEFT", &holdingRegisters[REG_STATE]);
  }
  else if (cmd == "d" || cmd == "RIGHT") {
    targetMode = 0;
    sendSPI("<RIGHT>");
    stringToMB("TURN_RIGHT", &holdingRegisters[REG_STATE]);
  }
  else {
    targetMode = 0;
    if (cmd.startsWith("<") && cmd.endsWith(">")) sendSPI(cmd.c_str());
    else {
      char buf[32]; snprintf(buf, sizeof(buf), "<%s>", cmd.c_str());
      sendSPI(buf);
    }
  }
}

bool targetA_State = false; // 00007 Coil state
...
void processModbus() {
  ...
  if (fc == 0x03) { // Read Holding
    ...
    resLen = 10;
    memcpy(res, req, 7); res[5] = 4; res[7] = 0x01; res[8] = 1;
    res[9] = targetA_State ? 0x01 : 0x00;
  }
  else if (fc == 0x05) { // Write Single Coil
    uint16_t val = (req[10] << 8) | req[11];
    memcpy(res, req, 12); res[5] = 6; resLen = 12;
    if (addr == 6) { // 00007
      targetA_State = (val == 0xFF00);
      char ctrl[21];
      MBToString(&holdingRegisters[REG_CONTROL], ctrl);
      if (targetA_State && strcmp(ctrl, "1") == 0) {
        handleCommand("1");
        targetA_State = false; // Trigger 후 리셋
        Serial.println("[MB] START MISSION: TargetA && Control=='1'");
      }
    }
  }
    ...
    if (addr >= REG_CONTROL && addr < REG_CONTROL + 10) {
        ...
        if (strlen(control) >= 1) {
            // "1" 이외의 방향키 등은 즉시 실행, "1"인 경우는 TargetA가 켜져 있어야 실행
            if (strcmp(control, "1") == 0) {
              if (targetA_State) {
                handleCommand("1");
                targetA_State = false; // 리셋
              }
            } else {
              handleCommand(control); // w, s, x 등 일반 명령은 즉각 실행
...
void setup() {
  ...
  Serial.printf("\nModbus Server for OPC UA: %s", ssid);
  ...
  mbServer.begin();
  stringToMB("IDLE", &holdingRegisters[REG_STATE]);
  Serial.println("=== ESP32 Modbus 0xxxx/4xxxx Ready ===");
}
...
void loop() {
  ...
  if (newMB) {
    ...
    mbClient = newMB;
    Serial.println("[MB] Gateway Connected");
  }

  if (mbClient.connected() && mbClient.available() > 0) processModbus();
  if (Serial.available() > 0) { char c = Serial.read(); char b[2]={c,0}; handleCommand(b); }

  updateOdometry();

  if (targetMode == 1 && travelDist >= 4000.0f) handleCommand("x");
  else if (targetMode == 3 && travelDist >= 300.0f) handleCommand("x");
  else if (targetMode == 4 && travelDist <= -300.0f) handleCommand("x");
