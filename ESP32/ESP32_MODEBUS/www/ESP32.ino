...
/**
 * @file ESP32.ino
 * @brief Integrated Master: Robot Control + Modbus TCP Server + Python Client
...
// Port 5000: Python Server (Client mode)
WiFiClient pyClient;
const char* pyHost = "192.168.0.36"; 
const uint16_t pyPort = 5000;
...
const unsigned long HBInterval = 50; // Heartbeat interval (ms)
...
void handleCommand(char cmd) {
  ...
  if (cmd == '1') {
    ...
    travelDist = 0.0f;
    sendSPI("<p>");
    stringToMB("RUNNING_PID", &holdingRegisters[REG_STATE]);
  }
  else if (cmd == 'w') {
    targetMode = 3;
    travelDist = 0.0f;
    sendSPI("<w>");
    stringToMB("MOVING_FWD", &holdingRegisters[REG_STATE]);
  }
  else if (cmd == 's') {
    targetMode = 4;
    travelDist = 0.0f;
    sendSPI("<s>");
    stringToMB("MOVING_BWD", &holdingRegisters[REG_STATE]);
  }
  else if (cmd == 'x') {
    targetMode = 0;
    sendSPI("<x>");
...
void processModbus() {
  ...
  if (fc == 0x03) { // Read
    ...
    if (addr >= REG_CONTROL && addr < REG_CONTROL + 10) {
        ...
        if (strlen(control) == 1) handleCommand(control[0]);
    }
  }
...
void setup() {
  ...
  mbServer.begin();
  stringToMB("IDLE", &holdingRegisters[REG_STATE]);
  Serial.println("=== ESP32 Integrated Master Ready ===");
}
...
void loop() {
  ...
  // Python Server Reconnect
  if (!pyClient.connected()) {
    static uint32_t lastPyTry = 0;
    if (now - lastPyTry > 5000) {
      lastPyTry = now;
      if (pyClient.connect(pyHost, pyPort)) Serial.println("[PY] Connected");
    }
  }
  ...
  if (pyClient.available() > 0) handleCommand(pyClient.read());
  ...
  } else if (targetMode == 3 && travelDist >= 300.0f) { // FWD 300
    handleCommand('x');
  } else if (targetMode == 4 && travelDist <= -300.0f) { // BWD 300
    handleCommand('x');
  }

  // Heartbeat & Data Sharing
  if (now - lastHBTime >= HBInterval) {
    ...
    if (pyClient.connected()) {
      pyClient.printf("<P,X:%.2f,Y:%.2f,T:%.3f>", posX, posY, posTheta);
    }

    // Debug Print
    Serial.printf("\rX:%.1f Y:%.1f T:%.1f Dist:%.1f Mode:%d  ", posX, posY, posTheta, travelDist, targetMode);
  }
