#include <WiFi.h>
#include <SPI.h>

/**
 * @file ESP32.ino
 * @brief Robot Control via Modbus TCP (Integrated for OPC UA Gateway)
 * @version 2.2
 * 
 * v2.2 변경사항:
 * - TargetA: Holding Register Bit(40007.0) → Coil(00007)로 변경
 * - Modbus FC01 (Read Coils), FC05 (Write Single Coil) 지원 추가
 * - KEPServerEX DeviceConnector v2.2와 호환
 * 
 * Location: c:\#PROJECT\NEW_MECHROBOT_V01\ESP32\ESP32_MODEBUS\ESP32.ino
 */

// ============== WiFi & Network Settings ==============
const char* ssid = "hhme";
const char* password = "hme*12345";

// Port 502: Modbus TCP Server
const uint16_t mbPort = 502;
WiFiServer mbServer(mbPort);
WiFiClient mbClient;

// ============== Hardware Pins ==============
const int chipSelectPin = 5;
const int enc1A = 32;
const int enc1B = 33;
const int enc2A = 25;
const int enc2B = 26;

// ============== Global Variables ==============
volatile long encoder1Count = 0;
volatile long encoder2Count = 0;

unsigned long lastHBTime = 0;
const unsigned long HBInterval = 100;

// Robot Physical Constants
const float WHEEL_DIAMETER_MM = 67.0f;
const float GEAR_RATIO = 30.0f;
const int ENCODER_CPR = 11;
const float TICKS_PER_REV = (float)ENCODER_CPR * 4.0f * GEAR_RATIO; // 1320
const float WHEEL_BASE_MM = 200.0f;
const float PI_F = 3.1415926535f;

float posX = 0.0f, posY = 0.0f, posTheta = 0.0f;
long lastEnc1 = 0, lastEnc2 = 0;

// Target Mode Variables
int targetMode = 0; 
volatile float travelDist = 0.0f; 

// ============== Modbus Holding Registers ==============
const int TOTAL_REGISTERS = 220; 
uint16_t holdingRegisters[TOTAL_REGISTERS] = {0};

// ============== Modbus Coils (v2.2 추가) ==============
const int TOTAL_COILS = 16;
bool coils[TOTAL_COILS] = {false};

enum CoilIndex {
  COIL_TARGET_A = 6      // Coil 00007 (0-based index = 6)
};

enum RegisterIndex {
  REG_POS_X_LO = 0,      
  REG_POS_X_HI = 1,      
  REG_POS_Y_LO = 2,      
  REG_POS_Y_HI = 3,      
  REG_POS_T_LO = 4,      
  REG_POS_T_HI = 5,      
  // REG_TARGET_A = 6,   // v2.2: Holding Register에서 제거, Coil로 이동
  REG_CONTROL = 99,      
  REG_STATE = 199        
};
const int STR_REG_LEN = 10; 

// ============== Encoder ISRs ==============
void IRAM_ATTR isr1A() {
  if (digitalRead(enc1A) == digitalRead(enc1B)) encoder1Count++;
  else encoder1Count--;
}
void IRAM_ATTR isr1B() {
  if (digitalRead(enc1A) != digitalRead(enc1B)) encoder1Count++;
  else encoder1Count--;
}
void IRAM_ATTR isr2A() {
  if (digitalRead(enc2A) == digitalRead(enc2B)) encoder2Count++;
  else encoder2Count--;
}
void IRAM_ATTR isr2B() {
  if (digitalRead(enc2A) != digitalRead(enc2B)) encoder2Count++;
  else encoder2Count--;
}

// ============== SPI Utility ==============
void sendSPI(const char *frame) {
  SPI.beginTransaction(SPISettings(1000000, MSBFIRST, SPI_MODE0));
  digitalWrite(chipSelectPin, LOW);
  size_t len = strlen(frame);
  for (size_t i = 0; i < len; ++i) {
    SPI.transfer((uint8_t)frame[i]);
  }
  digitalWrite(chipSelectPin, HIGH);
  SPI.endTransaction();
  Serial.printf("[SPI] Sent: %s\n", frame);
}

// ============== Modbus Utilities ==============
void floatToMB(float value, uint16_t* regs) {
  union { float f; uint8_t b[4]; } data;
  data.f = value;
  regs[0] = ((uint16_t)data.b[1] << 8) | data.b[0]; 
  regs[1] = ((uint16_t)data.b[3] << 8) | data.b[2]; 
}

void stringToMB(const char* str, uint16_t* regs) {
  int len = strlen(str);
  for (int i = 0; i < STR_REG_LEN; i++) regs[i] = 0;
  for (int i = 0; i < 20 && i < len; i++) {
    int idx = i / 2;
    if (i % 2 == 0) regs[idx] = (uint16_t)str[i] << 8;
    else regs[idx] |= (uint16_t)str[i];
  }
}

void MBToString(uint16_t* regs, char* str) {
  for (int i = 0; i < 20; i++) {
    int idx = i / 2;
    str[i] = (i % 2 == 0) ? (char)(regs[idx] >> 8) : (char)(regs[idx] & 0xFF);
  }
  str[20] = '\0';
}

// ============== Odometry ==============
void updateOdometry() {
  noInterrupts();
  long curE1 = encoder1Count;
  long curE2 = encoder2Count;
  interrupts();

  long dE1 = curE1 - lastEnc1;
  long dE2 = curE2 - lastEnc2;
  lastEnc1 = curE1;
  lastEnc2 = curE2;

  float dD1 = ((float)dE1 / TICKS_PER_REV) * (PI_F * WHEEL_DIAMETER_MM);
  float dD2 = ((float)dE2 / TICKS_PER_REV) * (PI_F * WHEEL_DIAMETER_MM);
  float dS  = (dD1 + dD2) * 0.5f;
  float dT  = (dD2 - dD1) / WHEEL_BASE_MM;

  if (targetMode != 0) travelDist += dS;

  float midT = posTheta + dT * 0.5f;
  posX += dS * cosf(midT);
  posY += dS * sinf(midT);
  posTheta += dT;

  while (posTheta > PI_F) posTheta -= 2.0f * PI_F;
  while (posTheta < -PI_F) posTheta += 2.0f * PI_F;
}

// ============== Unified Command Handler (Improved) ==============
void handleCommand(const char* cmdStr) {
  if (cmdStr == NULL || strlen(cmdStr) == 0) return;
  
  // Trim spaces or newlines
  String cmd = String(cmdStr);
  cmd.trim();
  if (cmd.length() == 0) return;

  Serial.printf("[CMD] Processing: %s\n", cmd.c_str());

  // Keyword Handling
  if (cmd == "1" || cmd == "MOVE") {
    Serial.println("Mission: PID Line Trace 4m");
    targetMode = 1;
    travelDist = 0.0f;
    sendSPI("<p>");
    stringToMB("RUNNING_PID_4M", &holdingRegisters[REG_STATE]);
  }
  else if (cmd == "x" || cmd == "STOP") {
    targetMode = 0;
    sendSPI("<x>");
    stringToMB("IDLE", &holdingRegisters[REG_STATE]);
  }
  else if (cmd == "w" || cmd == "FWD") {
    targetMode = 3;
    travelDist = 0.0f;
    sendSPI("<w>");
    stringToMB("MOVE_FWD", &holdingRegisters[REG_STATE]);
  }
  else if (cmd == "s" || cmd == "BWD") {
    targetMode = 4;
    travelDist = 0.0f;
    sendSPI("<s>");
    stringToMB("MOVE_BWD", &holdingRegisters[REG_STATE]);
  }
  else {
    // General Case: Handle raw strings
    targetMode = 0;
    // If it already has < and >, send as is. Else wrap it.
    if (cmd.startsWith("<") && cmd.endsWith(">")) {
      sendSPI(cmd.c_str());
    } else {
      // If it's a single char, use <c> format, otherwise just pass it through with <>
      char buf[32];
      snprintf(buf, sizeof(buf), "<%s>", cmd.c_str());
      sendSPI(buf);
    }
  }
}

// ============== Modbus Request Handler (v2.2 - Coil 지원) ==============
void processModbus() {
  uint8_t req[256];
  int len = mbClient.read(req, sizeof(req));
  if (len < 8) return;

  uint8_t fc = req[7];
  uint16_t addr = (req[8] << 8) | req[9];
  uint16_t qty = (req[10] << 8) | req[11];
  uint8_t res[256];
  int resLen = 0;

  // ========== FC01: Read Coils ==========
  if (fc == 0x01) {
    int byteCount = (qty + 7) / 8;
    resLen = 9 + byteCount;
    memcpy(res, req, 7);
    res[5] = 3 + byteCount;
    res[7] = 0x01;
    res[8] = byteCount;
    
    // Coil 데이터 패킹
    for (int i = 0; i < byteCount; i++) {
      res[9 + i] = 0;
      for (int bit = 0; bit < 8 && (i * 8 + bit) < qty; bit++) {
        int coilIdx = addr + i * 8 + bit;
        if (coilIdx < TOTAL_COILS && coils[coilIdx]) {
          res[9 + i] |= (1 << bit);
        }
      }
    }
    Serial.printf("[MB] FC01 Read Coils: addr=%d qty=%d\n", addr, qty);
  }
  // ========== FC05: Write Single Coil (v2.2 핵심 변경) ==========
  else if (fc == 0x05) {
    uint16_t coilValue = (req[10] << 8) | req[11];
    bool newValue = (coilValue == 0xFF00);
    
    if (addr < TOTAL_COILS) {
      bool prevValue = coils[addr];
      coils[addr] = newValue;
      
      Serial.printf("[MB] FC05 Write Single Coil: addr=%d value=%s\n", 
                    addr, newValue ? "ON" : "OFF");
      
      // TargetA (Coil 00007 = index 6) Rising Edge 감지
      if (addr == COIL_TARGET_A && newValue && !prevValue) {
        Serial.println("[MB] TargetA Rising Edge Detected!");
        handleCommand("1");
      }
    }
    
    // Echo back request (FC05 응답)
    memcpy(res, req, 12);
    res[5] = 6;
    resLen = 12;
  }
  // ========== FC15: Write Multiple Coils ==========
  else if (fc == 0x0F) {
    int byteCount = req[12];
    for (int i = 0; i < qty && i < TOTAL_COILS; i++) {
      int byteIdx = i / 8;
      int bitIdx = i % 8;
      bool prevValue = coils[addr + i];
      coils[addr + i] = (req[13 + byteIdx] >> bitIdx) & 0x01;
      
      // TargetA Rising Edge 감지
      if ((addr + i) == COIL_TARGET_A && coils[COIL_TARGET_A] && !prevValue) {
        Serial.println("[MB] TargetA Rising Edge Detected (FC15)!");
        handleCommand("1");
      }
    }
    
    memcpy(res, req, 12);
    res[5] = 6;
    resLen = 12;
    Serial.printf("[MB] FC15 Write Multiple Coils: addr=%d qty=%d\n", addr, qty);
  }
  // ========== FC03: Read Holding Registers ==========
  else if (fc == 0x03) {
    resLen = 9 + (qty * 2);
    memcpy(res, req, 7);
    res[5] = 3 + (qty * 2);
    res[7] = 0x03;
    res[8] = qty * 2;
    for (int i = 0; i < qty; i++) {
      res[9 + i*2] = (holdingRegisters[addr + i] >> 8) & 0xFF;
      res[10 + i*2] = holdingRegisters[addr + i] & 0xFF;
    }
  } 
  // ========== FC10: Write Multiple Registers ==========
  else if (fc == 0x10) {
    for (int i = 0; i < qty; i++) {
      holdingRegisters[addr + i] = (req[13 + i*2] << 8) | req[14 + i*2];
    }
    memcpy(res, req, 12);
    res[5] = 6;
    resLen = 12;

    // Trigger: Control String (40100)
    if (addr >= REG_CONTROL && addr < REG_CONTROL + 10) {
      char control[21];
      MBToString(&holdingRegisters[REG_CONTROL], control);
      if (strlen(control) >= 1) {
        handleCommand(control);
        // Auto-clear Control Tag after execution
        stringToMB("", &holdingRegisters[REG_CONTROL]);
      }
    }
  }

  if (resLen > 0) mbClient.write(res, resLen);
}

// ============== Setup ==============
void setup() {
  Serial.begin(115200);
  delay(1000);

  Serial.println("\n╔════════════════════════════════════════════════════════════╗");
  Serial.println("║   ESP32 Modbus Server v2.2 (OPC UA Integration)            ║");
  Serial.println("║   TargetA: Coil 00007 (FC01/FC05)                           ║");
  Serial.println("╚════════════════════════════════════════════════════════════╝");
  
  Serial.printf("\nConnecting to WiFi: %s", ssid);
  WiFi.begin(ssid, password);
  while (WiFi.status() != WL_CONNECTED) { delay(500); Serial.print("."); }
  Serial.println("\nWiFi Connected! IP: " + WiFi.localIP().toString());

  pinMode(enc1A, INPUT_PULLUP); pinMode(enc1B, INPUT_PULLUP);
  pinMode(enc2A, INPUT_PULLUP); pinMode(enc2B, INPUT_PULLUP);
  attachInterrupt(digitalPinToInterrupt(enc1A), isr1A, CHANGE);
  attachInterrupt(digitalPinToInterrupt(enc1B), isr1B, CHANGE);
  attachInterrupt(digitalPinToInterrupt(enc2A), isr2A, CHANGE);
  attachInterrupt(digitalPinToInterrupt(enc2B), isr2B, CHANGE);

  pinMode(chipSelectPin, OUTPUT); digitalWrite(chipSelectPin, HIGH);
  SPI.begin();

  mbServer.begin();
  stringToMB("IDLE", &holdingRegisters[REG_STATE]);
  
  Serial.println("\n┌─────────────────────────────────────────────────────────────┐");
  Serial.println("│ KEPServerEX Tag Configuration (v2.2)                        │");
  Serial.println("├─────────────┬──────────┬───────────┬───────────────────────┤");
  Serial.println("│ Tag Name    │ Address  │ Data Type │ Direction             │");
  Serial.println("├─────────────┼──────────┼───────────┼───────────────────────┤");
  Serial.println("│ POS_X       │ 40001    │ Float     │ Read (ESP32 → OPC)    │");
  Serial.println("│ POS_Y       │ 40003    │ Float     │ Read (ESP32 → OPC)    │");
  Serial.println("│ POS_T       │ 40005    │ Float     │ Read (ESP32 → OPC)    │");
  Serial.println("│ TargetA     │ 00007    │ Boolean   │ Write (OPC → ESP32)   │");
  Serial.println("│ Control     │ 40100.20H│ String    │ Write (OPC → ESP32)   │");
  Serial.println("│ State       │ 40200.20H│ String    │ Read (ESP32 → OPC)    │");
  Serial.println("└─────────────┴──────────┴───────────┴───────────────────────┘");
  
  Serial.println("\n=== ESP32 Modbus-OPC UA Ready ===");
}

// ============== Loop ==============
void loop() {
  uint32_t now = millis();

  WiFiClient newMB = mbServer.available();
  if (newMB) {
    if (mbClient) mbClient.stop();
    mbClient = newMB;
    Serial.println("[MB] OPC UA Gateway Connected");
  }

  if (mbClient.connected() && mbClient.available() > 0) processModbus();
  
  if (Serial.available() > 0) {
    char c = Serial.read();
    char buf[2] = {c, '\0'};
    handleCommand(buf);
  }

  updateOdometry();

  if (targetMode == 1 && travelDist >= 4000.0f) { // PID 4m
    handleCommand("x"); Serial.println("4m MISSION COMPLETE");
  } else if (targetMode == 3 && travelDist >= 300.0f) {
    handleCommand("x");
  } else if (targetMode == 4 && travelDist <= -300.0f) {
    handleCommand("x");
  }

  if (now - lastHBTime >= HBInterval) {
    lastHBTime = now;
    floatToMB(posX / 1000.0f, &holdingRegisters[REG_POS_X_LO]);
    floatToMB(posY / 1000.0f, &holdingRegisters[REG_POS_Y_LO]);
    floatToMB(posTheta * 180.0f / PI_F, &holdingRegisters[REG_POS_T_LO]);
    Serial.printf("X:%.3f Y:%.3f T:%.1f Dist:%.1f Mode:%d TargetA:%d\n", 
                  posX/1000.0f, posY/1000.0f, posTheta*180.0f/PI_F, 
                  travelDist, targetMode, coils[COIL_TARGET_A]);
  }

  yield();
}
