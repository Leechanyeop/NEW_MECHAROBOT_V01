...
void setup() {
  ...
  // SS 핀 테스트 (5회 토글)
  Serial.println(F("[TEST] Toggling SS pin 5 times..."));
  for (int i = 0; i < 5; i++) {
    digitalWrite(SPI_SS, LOW);
    delay(200);
    digitalWrite(SPI_SS, HIGH);
  ...
  Serial.println(F("[TEST] Done! Check slave serial."));
  
  // SCK, MOSI 핀 수동 테스트
  Serial.println(F("[TEST] Manual SCK/MOSI test..."));
  SPI.end();  // SPI 해제
  ...
  Serial.println(F("[TEST] If Slave Pin 11/13 LEDs blinked, wiring OK"));
  ...
  for (int i = 0; i < 5; i++) {
    ...
    Serial.print(F("  Sent 'T', Response: 0x"));
    Serial.println(response, HEX);
    delay(500);
  }
  Serial.println(F("[TEST] SPI test complete! Check Slave serial."));
}
...
void sendCommand(char cmd) {
  ...
  // SS LOW - Slave 선택 (더 긴 딜레이)
  digitalWrite(SPI_SS, LOW);
  delayMicroseconds(100);  // 100us로 증가
