#include "LocalQTRSensors.h"
#include <Arduino.h>

// 디지털 핀을 사용하여 커패시터가 방전되는 시간을 측정합니다 (RC 타입 설정)
...
// 아날로그 핀을 사용하여 전압(ADC 값)을 측정합니다 (아날로그 타입 설정)
...
// 센서 핀 배열과 총 개수를 설정하고 필요한 메모리를 할당합니다
...
// 아날로그 센서의 경우, 각 센서당 샘플링할 횟수를 설정합니다 (노이즈 감소용)
...
// 적외선 LED(Emitter) 제어 핀(단일)을 설정합니다. 측정 시에만 켜서 배터리를 절약합니다.
...
// 적외선 LED 제어 핀을 홀수/짝수로 나누어 설정합니다 (교차 점등 가능)
...
// 설정된 Emitter 핀을 해제하고 입력 모드로 전환합니다
...
// 홀수/짝수/전체 LED 중 원하는 그룹을 선택하여 켜고, 나머지는 끕니다
...
// 센서 값을 반복적으로 읽어 최솟값(Min)과 최댓값(Max)을 찾아 캘리브레이션을 수행합니다
...
// 캘리브레이션된 값(0~1000)을 읽습니다. 0은 최소반사(흰색), 1000은 최대반사(검은색)에 해당합니다.
...
// 실제 하드웨어 센서 값을 읽는 내부 함수입니다 (RC: 시간 측정, Analog: ADC 측정)
...
// 라인의 위치(0 ~ 7000)를 가중 평균 알고리즘으로 계산합니다. (readLineBlack/White의 내부 구현)
